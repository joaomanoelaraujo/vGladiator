...
import br.com.johnmanoel.d4rkk.glad.lobby.Lobby;
      ...
      if (evt.getWhoClicked().equals(this.player)) {
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getInventory())) {
          ...
          if (item != null && item.getType() != Material.AIR) {
            ...
            if (lobby != null && !JavaGlad.currentServerName.contentEquals(lobby.getServerName()) && lobby.getPlayers() < lobby.getMaxPlayers()) {
    ...
    for (Lobby lobby : Lobby.listLobbies()) {
      ItemStack icon = BukkitUtils.deserializeItemStack(lobby.getIcon().replace("{players}", StringUtils.formatNumber(lobby.getPlayers()))
          .replace("{maxplayers}", StringUtils.formatNumber(lobby.getMaxPlayers())).replace("{description}",
              StringUtils.formatColors(Lobby.CONFIG.getString("messages.description." + (JavaGlad.currentServerName.equals(lobby.getServerName()) ? "current" : "connect")))));
      if (JavaGlad.currentServerName.equals(lobby.getServerName())) {
